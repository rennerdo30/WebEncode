...
func (e *FFmpegEncoder) Probe(ctx context.Context, url string) (*ProbeResult, error) {
	...
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			return nil, fmt.Errorf("ffprobe failed: %s, stderr: %s", err, string(exitErr.Stderr))
		}
...
func (e *FFmpegEncoder) Transcode(ctx context.Context, task *TranscodeTask, progressCh chan<- Progress) error {
	...
	if err := cmd.Wait(); err != nil {
		...
		// If command failed, we want the stderr we were just parsing or a captured version of it.
		// Since we piped stderr, we can't get it from ExitError directly if we consumed it.
		// Use a multi-writer or capture last lines?
		// For simplicity, let's just return the error.
		// IMPORTANT: For proper error collection, we should capture the stderr buffer instead of just piping.
		// But since we are parsing it line by line, we can't easily get the last N lines unless we buffer them.
